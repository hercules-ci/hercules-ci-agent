...
withStore
  ...
  -> IO r
withStore = bracket (liftIO $ [C.block| refStore* {
...
ensurePath store path = [C.throwBlock| void {
...
finalizeDerivation = unsafePerformIO [C.exp|
    void (*)(Derivation *) {
      [](Derivation *v) {
        delete v;
      }
...
withHerculesStore wrappedStore =
   bracket (liftIO $ [C.block| refHerculesStore* {
...
printDiagnostics s = [C.throwBlock| void{
