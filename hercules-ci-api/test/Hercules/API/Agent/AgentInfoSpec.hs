...
objectV1 = AgentInfo {
  hostname = "a", agentVersion = "v",
  nixVersion = "nv", platforms = ["riscv-sel4"], systemFeatures = ["f1", "f2"], cachixPushCaches = ["c1", "c2"],
  substituters = ["s1", "s2"], concurrentTasks = 2 -- the hardcoded default
  }
...
objectV2 = AgentInfo {
  hostname = "a", agentVersion = "v",
  nixVersion = "nv", platforms = ["riscv-sel4"], systemFeatures = ["f1", "f2"], cachixPushCaches = ["c1", "c2"],
  substituters = ["s1", "s2"], concurrentTasks = 19 -- something else
  }
...
spec = describe "AgentInfo" $ do
  ...
  describe "ToJSON" $
    it "encodes v2 correctly" $ eitherDecode (encode objectV2) `shouldBe` Right objectV2
