...
-- | Should in a 'mask'ed section together with its construction.
move :: forall a. HasStdVector a => Ptr (CStdVector a) -> IO (StdVector a)
move ptr = StdVector <$> newForeignPtr cDelete ptr
...
fromList :: HasStdVector a => [a] -> IO (StdVector a)
fromList as = do
  vec <- Hercules.CNix.Std.Vector.new
  for_ as $ \a -> pushBack vec a
  pure vec

toList :: (HasStdVector a, Storable a) => StdVector a -> IO [a]
